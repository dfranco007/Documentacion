...
\section{Objetivos y necesidades del cliente}\label{obj-cliente} 
...
El alcance inicial de este proyecto está descrito en la sección \ref{obj-cliente}, sin embargo, el alcance del proyecto puede verse alterado en base al primer riesgo identificado que se explicará en la sección \ref{riesgos}. Debido a eso, se ha definido un alcance alternativo como recurso en caso de que se cumpla dicho riesgo. 

Por lo tanto, se procederá a cambiar la planificación completa del proyecto. Los objetivos a cumplir en ese caso serían los siguientes:

\begin{enumerate}
	\item Se haría un estudio de las diferentes técnicas de segmentación existentes de una forma más exahustiva, es decir, se estudiarían las diferentes optimizaciones o mejoras que se hayan realizado para cada técnica de segmentación. 
	\item Se buscarían implementaciones ya realizadas de manera eficiente de cada técnica de segmentación, de manera que se hiciera una tabla completa con las ventajas y las desventajas de cada una, poder probar estas implementaciones con diferentes imágenes si se pudiera, y comparar los resultados de la segmentación, tiempos de ejecución y rendimientos de cada una de éstas. Con todo ello se realizarían gráficas de los resultados obtenidos.
	\item Encontrada la mejor optimización de alguna de las diferentes técnicas se podría proceder a realizar su implementación eficaz, si la técnica no es muy compleja, en diferentes arquitecturas como NUMA, SMP o en GPUs. Se compararía lo realizado con las diferentes opciones encontradas en la anterior idea. 
...
EL costes de este proyecto ha sido únicamente la dedicación de horas humanas que se han dedicado para su realización. Esto se puede ver en la tabla \ref{dedicacionTemporal}. 
