...
\end{equation}
 ...
 En \cite{yong1}, antes de presentar el algoritmo se explican dos operaciones básicas que se ha de hacer en el algoritmo.
 
 La operación \textit{switch\_in()} para un punto x $\in L_{out}$ se define como:
 ...
 \end{itemize}
 Con esta operación se mueve el contorno un punto de la rejilla hacia fuera. Similarmente se define la operación \textit{switch\_out()} para un punto x $\in$ $L_{in}$:
...
La función F de velocidad presentanda antes, normalmente se suele separar en dos velocidades $F_{ext}$ que depende de los datos y $F_{in}$ para la realización de un suavizado del contorno.

Aclaradas las cuestiones  anteriores, el rápido algoritmo de \textit{level set} presentando en \cite{yong1} es el siguiente:
 ...
 \begin{table}[H]
 	...
 	\begin{tabular}{|l|}
		...
		\quad \quad \quad \quad \ \ \textit{switch\_in()} $\forall x \in L_{out}$ si F(x) $>$ 0. \\ 
		...
		\quad \quad \quad \quad - Evolución hacia fuera. Recorremos la lista $L_{in}$ y se hace  la operación \\ 
		\quad \quad \quad \quad \ \ \textit{switch\_out()} $\forall x \in L_{in}$ si F(x) $<$ 0. \\ 
		...
		\quad \quad \quad \quad - Se comprueba la condición de parada y si se satisface se continua al paso 3, \\ ç
		\quad \quad \quad \quad \ \ si no, seguiremos en el paso 2. \\
		\tabitem Paso 3:  Segundo ciclo donde se realiza un suavizado del contorno con un filtro Gaussiano. \\
 		\hline
 	\end{tabular}
 ...
 \begin{figure}[H]
 	\captionsetup{justification=centering}
 	\centering
 	\includegraphics[width=.7\textwidth]{./imagenes/switchLevelSet}
